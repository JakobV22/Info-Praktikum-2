...
void Kreuzung::vVerbinde(std::string sWeg1, std::string sWeg2,
	...
	//Test
	for (auto it = pKreuzung1->p_pWege.begin(); it != pKreuzung1->p_pWege.end(); it++) {
...
void Kreuzung::vAnnahme(std::unique_ptr<Fahrzeug> pFahrzeug,
	...
	else std::cout << "Gewünschte Kreuzung nicht angebunden" << std::endl;
...
std::shared_ptr<Weg> Kreuzung::pZufaelligerWeg(Weg &rWeg) {
	if (p_pWege.size() == 1) {
		return (p_pWege.front())->pGetRueckweg();					//falls nur 1 Weg, Rueckweg nehmen
		...
		for (auto it = p_pWege.begin(); it != p_pWege.end(); it++) {		//
			if ((*it)->vGetName().compare(rWeg.vGetName()) != 0) {			//falls Name von Weg aus Liste != Name des Hinweges
